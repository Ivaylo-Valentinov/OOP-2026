# Структури - 18.02.2026

## Принципи на ООП
1. **Абстракция**: Механизъм на изграждане на типове чрез тяхното описание без да е ясно как работят вътрешно.
...
2. `Event`, която да има член-данни `title`, `organizer`, `startTime` и `endTime`. `organizer` и `title` имат максимална дължина 128 символа.  
a) Дефинирайте член-функция `getDuration` към `Event`, която да връща продължителността на събитието в променлива от тип `Time`.  
